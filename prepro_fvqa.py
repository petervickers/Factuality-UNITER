...
def process_fvqa(jsonl, db, tokenizer, split_file, missing=None, ans2idx={}):
    id2len = {}
    ...
    for idx, data in tqdm(json_loaded.items(), desc='processing FVQA'):
        ...
        example['target'] = example['answer'].lower().translate(str.maketrans('', '', string.punctuation))
    ...
    with open_db() as db:
        with open(opts.annotation) as ann:
            if opts.missing_imgs is not None:
                ...
                missing_imgs = None
            id2lens, txt2img, idx2ans = process_fvqa(ann, db, tokenizer, opts.split_file, missing_imgs, ans2idx)
