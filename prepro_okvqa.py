...
def process_fvqa(jsona, jsonq, db, tokenizer, split, missing=None, ans2idx={}):
    id2len = {}
    ...
    for idx, data in tqdm(idxed_data.items(), desc='processing OKVQA'):
        example = data
        ...
        with open(opts.annotation0) as ann0:
            with open(opts.annotation1) as ann1:
                if opts.missing_imgs is not None:
                    ...
                    missing_imgs = None
                id2lens, txt2img, idx2ans = process_fvqa(ann0, ann1, db, tokenizer, opts.split, missing_imgs, ans2idx)
