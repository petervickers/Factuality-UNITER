...
class MemNN(nn.Module):
    def __init__(self, vocab_size, embd_size, ans_size, max_story_len, hops=3, dropout=0.2, te=True, pe=True):
        ...
        self.nn = nn.Linear(vocab_size, ans_size)
    ...
    def forward(self, x, q):
        ...
        if self.position_encoding:
            ...
            d = self.embd_size
            # Todo fix this
            pe = to_var(torch.zeros((J, d), dtype=torch.half)) # (s_sent_len, embd_size)
            #pe = to_var(torch.zeros((J, d)))
        ...
        for k in range(self.hops):
            ...
            if self.position_encoding:
                #print('type m', m.type(), 'type pe', pe.type())
