...
class CustomBertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, txt_embedding_kb):
        ...
        #print(hidden_states.shape, txt_embedding_kb.shape)
...
class CustomBertAttention(nn.Module):
    ...
    def forward(self, input_tensor, txt_embedding_kb):
        self_output, attention_probs = self.self(
            input_tensor, txt_embedding_kb
        )
...
class CustomBertLayer(nn.Module):
    ...
    def forward(self, hidden_states, txt_embedding_kb):
        attention_output, attention_probs = self.attention(
            hidden_states, txt_embedding_kb
        )
...
class CustomBertEncoder(nn.Module):
    ...
    def forward(
        self, txt_embedding, txt_embedding_kb,
    ):
        ...
        for idx in range(0, len(self.layer)):
            txt_embedding, txt_attention_probs = self.layer[idx](
                txt_embedding, txt_embedding_kb
            )
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        return (encoded_layers_t, pooled_output_t)


### CLASS DEFINITION ###
class KnowledgePooler(nn.Module):
    def __init__(self, kg_dim, final_dim):
        super(KnowledgePooler, self).__init__()
        self.dense = nn.Linear(kg_dim, final_dim)
        self.activation = nn.ReLU()
...
class QuestionGraphTransformer(nn.Module):
    def __init__(self, config, split, kg_dim, dropout_prob=0.1, default_gpu=True):
        super(QuestionGraphTransformer, self).__init__()
        config = BertConfig("config/bert_base_6layer_6conect.json")
        self.KGPooler = KnowledgePooler(kg_dim=kg_dim, final_dim=768)
        self.wordEmbedder = BertEmbeddings(config)
    ...
    def forward(
        ...
        txt_embedding = self.wordEmbedder(txt_ids)
        kg_embedding = self.KGPooler(kg_embedding_raw)
        
        #txt_attention_mask = txt_ids.cpu().detach().numpy() != 0
        #txt_attention_mask = txt_attention_mask.astype(int) 
        #txt_attention_mask = torch.from_numpy(txt_attention_mask).cuda()
        
        sequence_output_t, pooled_output_t = self.bert(
            txt_embedding,
            kg_embedding,
            output_all_encoded_layers=False,
        )

        return sequence_output_t, pooled_output_t
