...
from .data import DetectFeatTxtTokKgDataset, pad_tensors, get_gather_index


class KavqaDataset(DetectFeatTxtTokKgDataset):
    ...
    def __getitem__(self, i):
        ...
        return input_ids, img_feat, img_pos_feat, kg_embs, attn_masks, target_torch

def kavqa_collate(inputs):
    (input_ids, img_feats, img_pos_feats, kg_embs, attn_masks, targets
    ...
    kg_t = [torch.from_numpy(k) for k in kg_embs]
    kg_feat = pad_tensors(kg_t)   
...
class KavqaEvalDataset(KavqaDataset): 
    def __getitem__(self, i):
        qid = self.ids[i]

        example = DetectFeatTxtTokKgDataset.__getitem__(self, i)
        ...
        attn_masks = torch.ones(len(input_ids) + num_bb, dtype=torch.long)
        return qid, input_ids, img_feat, img_pos_feat, kg_embs, attn_masks, target_torch


def kavqa_eval_collate(inputs):
    (qids, input_ids, img_feats, img_pos_feats, kg_embs, attn_masks, targets
     ) = map(list, unzip(inputs))
