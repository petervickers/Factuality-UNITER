...
def process_vqa(jsona, jsonq, db, tokenizer, split, missing=None, ans2idx={}):
    ...
    for idx, data in tqdm(idxed_data.items(), desc='processing VQA'):
        ...
        best_answer_and_count = Counter([a['answer'] for a in example['answers']]).most_common(1)[0]        
        if best_answer_and_count[1] < 9:
            #print(f'Fewer than 9 annotators agreed, skipping...')
            skipped += 1
            continue
        ...
        best_answer = best_answer_and_count[0]
